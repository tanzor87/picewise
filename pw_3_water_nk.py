...
def pw_func_n(mg, mg1, mg2, nm, nb, nt, nu, km, kb, kt, ku):
    return np.piecewise(mg, [mg <= mg1, ((mg > mg1) & (mg <= mg2)), mg > mg2],
                        [
                            lambda mg: nm + nb * mg,
                            lambda mg: nm + nb * mg1 + nt * (mg - mg1),
                            lambda mg: nm + nb * mg1 + nt * (mg2 - mg1) + nu * (mg - mg2)
                        ])


def pw_func_k(mg, mg1, mg2, nm, nb, nt, nu, km, kb, kt, ku):
    return np.piecewise(mg, [mg <= mg1, ((mg > mg1) & (mg <= mg2)), mg > mg2],
                        [
                            lambda mg: km + kb * mg,
                            lambda mg: km + kb * mg1 + kt * (mg - mg1),
                            lambda mg: km + kb * mg1 + kt * (mg2 - mg1) + ku * (mg - mg2)
                        ])


def pw_combo(mg, mg1, mg2, nm, nb, nt, nu, km, kb, kt, ku):
    combo_data = combo(mg)
    data1 = combo_data[:len(mg)]
    data2 = combo_data[len(mg):]

    res1 = pw_func_n(data1, mg1, mg2, nm, nb, nt, nu, km, kb, kt, ku)
    res2 = pw_func_k(data2, mg1, mg2, nm, nb, nt, nu, km, kb, kt, ku)
